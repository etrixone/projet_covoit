...
@if(!empty($trajetsEtapesDepart ))

<h4><strong>Voici les Trajets qui font etapes la ou vous partez</strong></h4>               
...
<div class="container">
    ...
    <div class="row cadre">
        <div class="col-sm-4 center">
            {{ Carbon\Carbon::parse($trajetsEtapesDepart->TRJ_DATE_DEPART)->format('d-m-Y') }}<br>
            
            <span class="arial-bold" >1.</span> <span class="arial-bold rouge">Départ : </span> <span class="arial-bold" >{{ $trajet->TRJ_DEPART }}</span>
            <div class="glyphicon glyphicon-arrow-right"></div><br>
            {{ Carbon\Carbon::parse($trajetsEtapesDepart->TRJ_HEURE_DEPART)->format('H:i') }}
        ...
        <div class="col-sm-4 center">
            ...
            <span class="arial-bold" >2.<span class="arial-bold rouge">Etape(s) : </span>
            <br>
            @if(!empty($trajetsEtapesDepart->TRJ_ETAPE1))
            <span class="arial-bold" >{{ $trajetsEtapesDepart->TRJ_ETAPE1 }} </span>
            <span class="arial-bold" >{{ $trajetsEtapesDepart->TRJ_ETAPE2 }} </span>
            <span class="arial-bold" >{{ $trajetsEtapesDepart->TRJ_ETAPE3 }} </span>
            @else <span class="arial-bold" >Pas d'étape</span>
            @endif
        </div>
        ...
        <div class="col-sm-4 center">
            <br>
            <div class="glyphicon glyphicon-arrow-right"></div>
            <span class="arial-bold" >3.<span class="arial-bold rouge">Destination : </span> <span class="arial-bold" >{{ $trajetsEtapesDepart->TRJ_DESTINATION }}</span><br>
            10:30
